...
function searchLayer() {
    // This project uses jQuery bPopup binding for .eSearch (below).
    // Avoid double-binding native listeners which can break clicks.
    if (window.jQuery && window.jQuery.fn && window.jQuery.fn.bPopup) {
        return;
    }
    ...
    btnSearchs.forEach( function(btnSearch) {
        btnSearch.addEventListener('click', function(){
            // Mobile: focus the always-visible header search
            if (window.matchMedia && window.matchMedia('(max-width:768px)').matches) {
                var inlineInput = document.querySelector('#quick_search #keyword');
                if (inlineInput) {
                    window.scrollTo({ top: 0, behavior: 'smooth' });
                    setTimeout(function(){ inlineInput.focus(); }, 200);
                    return;
                }
            }

            // Desktop/legacy: open search layer
...
jQuery(document).ready(function() {
	...
	/* 검색: .eSearch (PC/모바일 공통)
	   - skin3는 모바일에서 로고 아래 #quick_search 검색창을 항상 노출(add_layout.css)
	   - 따라서 "아이콘 클릭 = 검색창 열기"가 아니라 "검색창으로 스크롤 + 포커스"가 기본 동작
	   - PC에서는 기존대로 bPopup(레이어) 사용, 단 bPopup 미로딩/오류 시에도 fallback 동작 보장
	   - 모바일에서 click이 누락되는 경우가 있어 pointerup/touchend도 함께 수신
	   - touchend + click 중복 실행은 time-guard로 방지
	*/
	(function(){
		var lastFireAt = 0;
		var EVENTS = 'click.kreamSearch touchend.kreamSearch pointerup.kreamSearch';
		function getQuickSearchInput(){
			return (
				document.querySelector('#quick_search input[name="keyword"]') ||
				document.querySelector('#quick_search #keyword') ||
				document.querySelector('#quick_search input[type="search"]') ||
				document.querySelector('#quick_search input[type="text"]')
			);
		}
		...
		function isQuickSearchVisible(){
			var qs = document.getElementById('quick_search');
			if (!qs) return false;
			var style = window.getComputedStyle ? window.getComputedStyle(qs) : null;
			if (!style) return !!qs.offsetParent;
			return style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0';
		}
		function focusInlineQuickSearch(){
			var qs = document.getElementById('quick_search');
			var input = getQuickSearchInput();
			if (!qs || !input) return false;

			// add a short highlight so user feels the tap worked
			qs.classList.add('kream_flash');
			window.scrollTo({ top: 0, behavior: 'smooth' });
			setTimeout(function(){
				try { input.focus(); } catch (err) {}
				setTimeout(function(){
					try { qs.classList.remove('kream_flash'); } catch (err) {}
				}, 700);
			}, 220);
			return true;
		}

		jQuery(document).off(EVENTS, '.eSearch').on(EVENTS, '.eSearch', function(e) {
			// Avoid double fire (touchend -> click)
			var now = Date.now();
			if (now - lastFireAt < 450) return;

			// Ignore mouse pointerup (desktop will use click)
			if (e && e.type === 'pointerup' && e.originalEvent && e.originalEvent.pointerType === 'mouse') {
				return;
			}

			lastFireAt = now;

			// Prevent default for anchors/buttons so it doesn't jump or submit
			if (e && e.preventDefault) e.preventDefault();
			if (e && e.stopPropagation) e.stopPropagation();

			// If mobile-like OR quick_search already visible (skin3 mobile UX): just focus inline search
			if (isMobileLike() || isQuickSearchVisible()) {
				if (focusInlineQuickSearch()) return;
			}

			// Desktop: open popup if bPopup available
			if (jQuery.fn && jQuery.fn.bPopup) {
				jQuery('#quick_search').bPopup({
					...
					onOpen: function() {
						...
						// ensure focus after popup open
						setTimeout(function(){
							try {
								var el = getQuickSearchInput() || document.querySelector('#keyword');
			...
			// Fallback (no bPopup): show element and focus input
			try {
				var qs = document.getElementById('quick_search');
				if (qs) qs.style.display = 'block';
			} catch (err) {}
			focusInlineQuickSearch();
		});
	})();
